...
const waitForServiceWorkerCleanup = async (
...
const forceRefreshWithCacheBust = (): void => {
  try {
    ...
    console.error(
      "Failed to create cache-bust URL, using simple reload:",
      error
    );
...
const cleanupServiceWorkers = async (): Promise<{
  removed: number;
  ...
  if (failures.length > 0) {
    failures.forEach(({ result, index }) => {
      const error = new Error(
        `Failed to unregister service worker: ${registrations[index].scope}`
      );
      console.error(error);
    });
  }
  ...
  const successfullyUnregisteredCount = unregisterResults.filter(
...
const cleanupIndexedDB = async (): Promise<number> => {
  ...
  try {
    ...
    if ("databases" in indexedDB) {
      ...
      console.log(
        "indexedDB.databases() not supported, using known database names"
      );
      databasesToDelete = [
        "serwist-expiration",
        "workbox-expiration",
    ...
    console.log(
      "Deleting service worker related databases:",
      databasesToDelete
    );

    const deletePromises = databasesToDelete.map(
      (dbName) =>
...
const cleanupStorage = (storage: Storage, storageName: string): number => {
  try {
    ...
    try {
      ...
      console.log(
        `${storageName} is not accessible due to security restrictions:`,
        securityError
      );
      return 0;
    }
    ...
    if (swRelatedKeys.length > 0) {
      console.log(
        `Clearing service worker related ${storageName} items:`,
        swRelatedKeys
      );
      swRelatedKeys.forEach((key) => {
        try {
          ...
          console.warn(
            `Failed to remove ${storageName} key '${key}':`,
            removeError
          );
        }
      });
...
export const removeAllServiceWorkers = async (): Promise<void> => {
  ...
  try {
    ...
    if ("localStorage" in window) {
      stats.localStorageKeysRemoved = cleanupStorage(
        localStorage,
        "localStorage"
      );
    }
    ...
    if ("sessionStorage" in window) {
      stats.sessionStorageKeysRemoved = cleanupStorage(
        sessionStorage,
        "sessionStorage"
      );
    }
    ...
    if (stats.serviceWorkersRemoved > 0) {
      console.log(
        "🔄 Refreshing page to load fresh content after service worker removal..."
      );
    ...
    if (stats.serviceWorkersRemoved > 0) {
      console.log(
        "🔄 Refreshing page despite errors to attempt fresh content load..."
      );
      setTimeout(() => {
        forceRefreshWithCacheBust();
